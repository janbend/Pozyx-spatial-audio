...
String ip_addr = "127.0.0.1";     // change the IP-address for OSC messages
int oscPort = 8888;               // change this to your UDP port

double room_x = 6200.0;           // change length of tracking area in x-axis in m²
double room_y = 2650.0;           // change length of tracking area in y-axis in m²
...
// initialization of kalman filter variables and values
double Rx = 4.0;    // measured noise covariance in x-axis
double Qx = 0.002;  // process noise covariance in x-axis
double Ry = 5.;     // measured noise covariance in y-axis 
double Qy = 0.0015; // process noise covariance in y-axis
double Rz = 100.0;  // measured noise covariance in z-axis 
double Qz = 0.3;    // process noise covariance in z-axis
...
double pos_rx; // former int value, use double when kalman filter
double pos_ry;
double pos_rz;
double pos_lrx;
double pos_lry;
...
// setup serial input of workstation tag
  if (serial) {
    try {
  ...
  // creates gui toggles for osc control messages
  ...
  // load compass reference image from data folder
  ...
  // creates slider element in gui to optimize kalman-filter gain
...
void draw() {
  ...
  // calculation of discrete kalman filter
  Xk = pos_rx;
  ...
  // send osc message to control digital signage devices (e.g. brightsign)
  if (brightsign.getBooleanValue()) {
    OscMessage newMessage = new OscMessage("start");  
    oscP5.send(newMessage, myRemoteLocation);
  } 

  // send serperate osc messages for orientation & position of remote tag
  // modification orientation valuesmay be required for different head rotation methods
  ...
  // send osc messages for position of remote tag    
  ...
  // send osc messages to dearvr for cartesian mode
  ...
  // send osc messages if tag in specific roomzone
  ...
  text("(c) Pozyx Labs", width-120, 20);
